...
- **概括训练**：四步向导（通读材料 → 找点 → 归类命名 → 成句对照）；内置来源型材料已移除，请在“我的材料”中提供自己有权使用的材料，再由 AI 生成题目
- **规范改写**：口语化表达转正式公文语体，支持离线题库和在线 AI 出题
- **我的材料**：粘贴或上传材料，由 AI 生成题目、切片、采分点和参考答案；原始材料与 AI 加工结果分开保存
- **两种批改模式**：用户材料生成后可离线对照参考答案，或接入 AI 做语义级逐点批改
- **错题本**：自动记录错误，支持优先重练
...
概括训练不再内置来源型材料，支持导入个人材料和完整真题，由 AI 生成可练习结构。规范改写仍提供离线题库。
